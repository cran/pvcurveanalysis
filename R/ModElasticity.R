...
#' point is then fitted linearly and after transformation of RWD (%) to RWC the modulus of elasticity (M.Elasticity) equals the
#' slope of the fitted line. \cr \cr
...
#' \item{modulus.elasticity}{modulus of elasticity (MPa)}
#' \item{formula}{formula of the transformed linear osmotic potential fit (1/-MPa) and the pressure potential (MPa) fit}
#' \item{coef}{coefficients of the osmotic (1/-MPa) and pressure potential (MPa) fit}
  ...
  for (i in 1:length(sample_id)) {
    ...
    if (length(press.pot.before.tlp[, 1]) < 3) {
      ...
      try({
        ...
        # calculate slope of fit, after transferring RWD to RWC, which is bulk modulus of elasticity
        elasticity <-
          (press.lin.data[1] - press.lin.data[2]) / ((100-pot_data$x[1])/100 - (100-pot_data$x[2])/100)
        ...
        if (graph == TRUE) {
          suppressWarnings(
            PlotOutput(
              ...
              y.axis = expression(paste(Psi, ' ' , (MPa))),
              x.intercept = tlp_i,
