...
#'
#' @param data data frame, at least with a column containing numeric water potential (MPa), fresh.mass (g) and dry.mass (g) values,
...
#' @param water.potential optional name of the column in data containing the water potential values (MPa), default = "water.potential"
#' @param fresh.mass optional name of the column in data containing the fresh mass values (g), default: "fresh mass"
#' @param dry.mass
...
#' @return the original data frame (data) extended by a numeric column containing the saturated fresh mass values ("fresh.mass.saturated")
...
FMSaturated <- function(data,
  ...
  data_in <-
    data.frame(data_in[[sample]], data_in[[fresh.mass]], data_in[[water.potential]],
               data_in[[dry.mass]])
  names(data_in) <-
    c(
      paste0(sample),
  ...
  data_in$leaf.water <-
    data_in[[fresh.mass]] - data_in[[dry.mass]]
  ...
  for (i in 1:length(unique(data_in$sample))) {
    ...
    if (length(tlp) == 0) {
      ...
      data_in_subset_sub <-
        data.frame(fresh.mass = data_in_subset_sub[[fresh.mass]],
      ...
      if (length(data_in_subset_sub[, 1]) < 3) {
        ...
        m <-
          lm(fresh.mass ~ water.potential, data = data_in_subset_sub)



        # saturated water content is where water.potential of the linear regression line is 0, e.g. intercept of linear regression
        sat.FM_i <- as.numeric(coef(m)[1])
      }
    }

    sat.FM <-
      c(sat.FM, rep(sat.FM_i, times = length(data_in_subset[, 1])))   # repeat result  as often as necessary in order to add to data frame

  }


  return(data.frame(data, fresh.mass.saturated = sat.FM))
}
